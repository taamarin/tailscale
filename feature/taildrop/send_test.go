...
func (m *mockFileOps) Rename(oldPath, finalName string) (string, error) {
	...
	// Try finalName, then finalName (1), (2), … on collision.
	name := finalName
	for i := 0; ; i++ {
		dst := filepath.Join(m.root, name)
		if _, err := os.Stat(dst); err == nil {
			// Already exists → pick the next “foo (1)” style name and retry.
			name = nextFilename(finalName)
			continue
		} else if !os.IsNotExist(err) {
			return "", fmt.Errorf("stat %q: %w", dst, err)
		}
		// dst does not exist, so we're safe to rename
...
// strip the "uri://" test prefix if present, then return the basename
