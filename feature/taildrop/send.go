...
func (m *manager) PutFile(id clientID, baseName string, r io.Reader, offset, length int64) (int64, error) {
	...
	case m.opts.Dir == "" && m.opts.FileOps == nil:
		return 0, ErrNoTaildrop
	...
	dstID := m.opts.FileOps.Join(m.opts.Dir, baseName)
	// and make sure we don't delete it while uploading:
	m.deleter.Remove(m.opts.FileOps.Base(dstID))
	...
	wc, partialPath, err := m.opts.FileOps.OpenWriter(dstID, offset, 0o666)
	if err != nil {
	...
	defer func() {
		wc.Close()
		if err != nil {
			m.deleter.Insert(m.opts.FileOps.Base(partialPath)) // mark partial file for eventual deletion
		}
	}()
	...
	newID, err := m.opts.FileOps.Rename(partialPath, baseName)
	if err != nil {
