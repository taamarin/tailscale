...
func (e *extension) consumeEventbusTopics() {
	reqSub := eventbus.Subscribe[magicsock.UDPRelayAllocReq](e.eventClient)
	respPub := eventbus.Publish[magicsock.UDPRelayAllocResp](e.eventClient)
	...
	for {
		...
		case req := <-reqSub.Events():
			...
			respPub.Publish(magicsock.UDPRelayAllocResp{
				ForNodeKey:  req.ReceivedFromNodeKey,
				ForDiscoKey: req.ReceivedFromDiscoKey,
